...
class TerminalNotebook:
    ...
    def __init__( self, notebook = None, wtree = None, prefs = None): # all arrays follow the same TABS order
        ...
        dprint("NOTEBOOK: get & connect to vadjustments")
        #dprint(TABS[:-1])
        for x in TABS[:-1]:
            #dprint(x)
        ...
        #dprint(self.vadjustment)
        #dprint(self.vhandler_id)
        #dprint(self.auto_scroll)
    ...
    def get_tab_list(self):
        ...
        self.visible_tablist = []
        tab_num = 0
        #dprint("NOTEBOOK: get_tab_list -- self.tab_showing")
        ...
        for tab in self.tab_showing:
            ...
            tab_num += 1
        dprint("NOTEBOOK: get_tab_list() new self.visible_tablist: %s" % self.visible_tablist)
        return
    ...
    def show_tab(self, tab):
        ...
        self.get_tab_list()
        dprint("NOTEBOOK: self.visible_tablist %s" % self.visible_tablist)
    ...
    def hide_tab(self, tab):
        ...
        if tab == TAB_WARNING:
            ...
            pos = self.notebook.page_num(self.queue_tab)
        if pos is not -1:
            dprint("NOTEBOOK: hiding tab %s, pos %s." % (tab, pos))
    ...
    def set_scroll(self,  vadjustment):
        ...
        #dprint("NOTEBOOK: set_scroll() -- vadjustment")
    ...
    def switch_page(self, notebook, page, page_num):
        ...
        dprint("NOTEBOOK: switch_page(); page_num = " + str(page_num))
    ...
    def overwrite(self, num, text, tagname = None):
        ...
        if iter.get_chars_in_line() >= 7:
            ...
            dprint("*** Notebook: overwrite: less than 7 chars in line... no linenumber???")
    ...
    def parse_escape_sequence(self, sequence = "[39;49;00m"):
        ...
        """
        #dprint("Notebook: parse_escape_sequence(): parsing '%s'" % sequence)
        if sequence.startswith("[") and sequence.endswith("m"):
            ...
            for item in terms:
                try:
                    ...
                    dprint("Notebook: parse_escape_sequence(); failed to convert item '%s' to an integer" % item)
                    return False
                if 0 <= item <= 1:
                    ...
                    dprint("Notebook: parse_escape_sequence(): ignoring term '%s'" % item)
            ...
            if not self.current_tagnames:
                self.current_tagnames = ['default']
            #dprint("Notebook: parse_escape_sequence(): tagnames are %s" % self.current_tagnames)
            return True
            ...
            dprint("Notebook: parse_escape_sequence(): unsupported escape sequence '%s'" % sequence)
            return False
