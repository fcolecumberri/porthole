...
class ProcessOutputReader(threading.Thread):
    ...
    def run(self):
        ...
        dprint("PROCESS_READER: ProcessOutputReader(); process id = %d" %os.getpid())
        char = None
        while not self.die:
                ...
                if self.process_running and (self.fd != None):
                    try:
                        ...
                        if e.args[0] == 5: # 5 = i/o error
                            dprint("PROCESS_READER: ProcessOutputReader: process finished, closing")
                            try:
                                dprint("PROCESS_READER: is self.fd a tty? '%s'" % os.isatty(self.fd))
                                os.close(self.fd)
                                dprint("PROCESS_READER: ProcessOutputReader: closed okay")
                            except Exception, e:
                                # probably already closed
                                dprint("PROCESS_READER: ProcessOutputReader: couldn't close self.fd. exception: %s" % e)
                        else:
                            # maybe the process died?
                            dprint("PROCESS_READER: ProcessOutputReader: .fd OSError: %s" % e)
                        char = None
                    ...
                    try:
                        ...
                        dprint("PROCESS_READER: ProcessOutputReader: .f OSError: %s" % e)
                        # maybe the process died?
                        char = None
                if char:
                    ...
                    self.process_running = False
                    while self.string != "":
                        #dprint("PROCESS_READER: ProcessOutputReader: waiting for update to finish")
